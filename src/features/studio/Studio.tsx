...
import { ArrowRight, Check, ChevronRight, Clock, Palette, ShieldCheck, Users, Wallet, Shirt, ClipboardList, Settings, ShoppingBag, LogOut, Camera, Upload, Paintbrush, Trash2, Archive, ArchiveRestore } from 'lucide-react';
import TopNav from '@/components/navigation/TopNav';
import Footer from '@/components/layout/Footer';
import { Action, Actor, Stage, stageLabels, transition, canGenerate, canDeleteDesign, splitEarnings, publicationReady } from './workflow';
...
function DesignList({ compact = false }: { compact?: boolean }) {
  const { designs, deleteDesign, archiveDesign, unarchiveDesign } = useDesigns();
  const { pathname } = useLocation();
  const base = pathname.startsWith('/creator') ? '/creator' : '/parent';
  const [confirmId, setConfirmId] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);
  const active = designs.filter(d => !d.archived_at);
  const archived = designs.filter(d => !!d.archived_at);

  async function handleRemove(d: Design) {
    setBusy(true);
    if (canDeleteDesign(d.stage)) { await deleteDesign(d.id); }
    else { await archiveDesign(d.id); }
    setBusy(false); setConfirmId(null);
  }
  async function handleUnarchive(id: string) { setBusy(true); await unarchiveDesign(id); setBusy(false); }

  const confirmDesign = designs.find(d => d.id === confirmId);
  const isDeletion = confirmDesign ? canDeleteDesign(confirmDesign.stage) : false;

  return <>{!compact && <><h1>My designs</h1><p className="studio-lead">See where every idea is on its way to a shirt.</p></>}
    <Panel title={compact ? 'Your designs' : undefined}>{active.length ? active.map(d => <div key={d.id} className="studio-row-wrapper"><Link className="studio-row" to={`${base}/design/${d.id}`}><span><strong>{d.title}</strong></span><span className="studio-badge">{stageLabels[d.stage]}</span><ChevronRight size={18} /></Link><button className="studio-row-action" title={canDeleteDesign(d.stage) ? 'Delete design' : 'Archive design'} onClick={(e) => { e.preventDefault(); setConfirmId(d.id); }}>{canDeleteDesign(d.stage) ? <Trash2 size={16} /> : <Archive size={16} />}</button></div>) : <Empty title="The first idea starts here">Submit a drawing. You&#39;ll see every step in this space.</Empty>}</Panel>
    {!compact && archived.length > 0 && <Panel title="Archived designs">{archived.map(d => <div key={d.id} className="studio-row-wrapper archived"><Link className="studio-row" to={`${base}/design/${d.id}`}><span><strong>{d.title}</strong></span><span className="studio-badge">{stageLabels[d.stage]}</span></Link><button className="studio-row-action" title="Unarchive" disabled={busy} onClick={() => handleUnarchive(d.id)}><ArchiveRestore size={16} /></button></div>)}</Panel>}
    {confirmId && confirmDesign && <div className="studio-overlay" onClick={() => setConfirmId(null)}><div className="studio-confirm" onClick={e => e.stopPropagation()}><h2>{isDeletion ? 'Delete this design?' : 'Archive this design?'}</h2><p>{isDeletion ? 'This will permanently remove the drawing and any generated artwork. This cannot be undone.' : 'This design will move to your archived section. If it was published, it will continue to be available in the Rendered Youth library.'}</p><div className="studio-actions"><Button disabled={busy} onClick={() => handleRemove(confirmDesign)}>{busy ? 'Working...' : isDeletion ? 'Delete permanently' : 'Archive design'}</Button><button className="studio-link" onClick={() => setConfirmId(null)}>Cancel</button></div></div></div>}
  </>;
}
function Submit() {
  const { session } = useAuth();
...
function DesignDetail({ id }: { id: string }) {
  ...
  const { deleteDesign, archiveDesign } = useDesigns();
  const navigate = useNavigate();
  const [showRemoveConfirm, setShowRemoveConfirm] = useState(false);
  const [removebusy, setRemovebusy] = useState(false);
  const isDeletable = canDeleteDesign(d.stage);
  const isArchived = !!d.archived_at;
  async function handleRemoveDesign() {
    setRemovebusy(true);
    if (isDeletable) { await deleteDesign(id); }
    else { await archiveDesign(id); }
    setRemovebusy(false); setShowRemoveConfirm(false);
    navigate(`${base}/designs`);
  }

  return <><Link className="studio-link" to={`${base}/designs`}>← My designs</Link><h1>{d.title}</h1><span className="studio-badge">{stageLabels[d.stage]}</span>{isArchived && <span className="studio-badge" style={{ background: '#f0e6d0', color: '#8a6d3b', marginLeft: '8px' }}>Archived</span>}<p className="studio-lead">{d.story}</p>
    ...
    {!isArchived && <Panel title={isDeletable ? 'Delete this design' : 'Archive this design'}><p>{isDeletable ? 'Permanently remove this design and any generated artwork.' : 'Move this design to your archive. Published artwork will remain in the Rendered Youth library.'}</p><button className="studio-link" style={{ color: isDeletable ? '#c00' : '#8a6d3b' }} onClick={() => setShowRemoveConfirm(true)}>{isDeletable ? <><Trash2 size={14} /> Delete design</> : <><Archive size={14} /> Archive design</>}</button></Panel>}
    {showRemoveConfirm && <div className="studio-overlay" onClick={() => setShowRemoveConfirm(false)}><div className="studio-confirm" onClick={e => e.stopPropagation()}><h2>{isDeletable ? 'Delete this design?' : 'Archive this design?'}</h2><p>{isDeletable ? 'This will permanently remove the drawing and any generated artwork. This cannot be undone.' : 'This design will move to your archived section. If it was published, it will continue to be available in the Rendered Youth library.'}</p><div className="studio-actions"><Button disabled={removebusy} onClick={handleRemoveDesign}>{removebusy ? 'Working...' : isDeletable ? 'Delete permanently' : 'Archive design'}</Button><button className="studio-link" onClick={() => setShowRemoveConfirm(false)}>Cancel</button></div></div></div>}</>;
}
...
function CreatorOverview() {
  ...
  const attention = designs.filter(d => !d.archived_at && ['choose', 'changes', 'parent_approval'].includes(d.stage));
  const activeDesigns = designs.filter(d => !d.archived_at);
  return <><p className="studio-eyebrow">YOUR CREATOR STUDIO</p><h1>Let's make something great.</h1><p className="studio-lead">Your designs, your ideas, the next little step.</p><div className="studio-stats">{[[activeDesigns.length, 'Designs'], [activeDesigns.filter(d => d.stage === 'published').length, 'Live designs'], [activeDesigns.filter(d => ['review', 'mockup', 'ready'].includes(d.stage)).length, 'In review'], ['$0.00', 'Paid earnings']].map(([n, label]) => <Panel key={label}><strong className="studio-stat">{n}</strong><span>{label}</span></Panel>)}</div><Panel title="Your next step">{attention.length ? attention.map(d => <DesignRow key={d.id} design={d} />) : <><p>{designs.length ? "You're all caught up. We'll show the next action here when a design is ready." : 'Ready to start? Submit your first drawing.'}</p><Link className="studio-button" to="/creator/submit">Submit a drawing</Link></>}</Panel>{!profile?.payout_ready && <Notice><strong>One more step before selling:</strong> connect your payout account. <Link to="/creator/earnings">View payout setup →</Link></Notice>}<DesignList compact /></>;
}
...
function Admin({ path }: { path: string }) {
  ...
  const { designs, updateDesign, reload } = useDesigns();  const { artists } = useArtists();
  ...
  const queue = designs.filter(d => !d.archived_at && (['review', 'mockup', 'ready'].includes(d.stage) || d.request_note));
  const archivedDesigns = designs.filter(d => !!d.archived_at);
  const [adminDeleteId, setAdminDeleteId] = useState<string | null>(null);
  const [adminDeleting, setAdminDeleting] = useState(false);
  async function adminPermanentDelete(id: string) {
    setAdminDeleting(true);
    await supabase.from('design_mockups').delete().eq('design_id', id);
    await supabase.from('designs').delete().eq('id', id);
    setAdminDeleting(false); setAdminDeleteId(null);
    reload();
  }
  return <><p className="studio-eyebrow">THE NEXT RIGHT ACTION</p><h1>{path === '/admin' ? 'Today at Rendered Youth' : 'Designs & approvals'}</h1><p className="studio-lead">Review the idea, prepare the product, and help each artist move forward.</p><div className="studio-stats">{[['Review artwork', designs.filter(d => d.stage === 'review' && !d.archived_at).length], ['Prepare products', designs.filter(d => d.stage === 'mockup' && !d.archived_at).length], ['Ready to publish', designs.filter(d => d.stage === 'ready' && !d.archived_at).length], ['Extra generation requests', designs.filter(d => d.request_note && !d.archived_at).length]].map(([s, n]) => <Panel key={String(s)}><strong className="studio-stat">{n}</strong><span>{s}</span></Panel>)}</div>{!queue.length && <Empty title="Your queue is clear">Create a drawing in the family view to try the review workflow.</Empty>}{queue.map(d => <Panel key={d.id} title={d.title}><span className="studio-badge">{stageLabels[d.stage]}</span><p>{d.story}</p><p>{d.selected_option >= 0 ? `Family selected: ${sampleStyles[d.selected_option]}` : 'Family has not selected artwork yet.'}</p>{d.request_note && <Notice><strong>Extra generation request:</strong> {d.request_note}<div className="studio-actions"><Button onClick={() => updateDesign(d.id, { admin_credits: d.admin_credits + 1, request_note: null } as any)}>Approve one extra batch</Button><button className="studio-link" onClick={() => updateDesign(d.id, { request_note: null, admin_note: 'Additional generation not approved.' } as any)}>Decline</button></div></Notice>}{d.stage === 'review' && <><Field label="Review note / changes needed"><textarea maxLength={500} value={note} onChange={e => setNote(e.target.value)} /></Field><div className="studio-actions"><Button onClick={() => updateDesign(d.id, { stage: 'mockup' } as any)}>Approve art & prepare product</Button><Button disabled={!note.trim()} onClick={() => updateDesign(d.id, { admin_note: note, stage: 'changes' } as any)}>Request changes</Button></div></>}{d.stage === 'mockup' && <><p>Live preparation will validate the print file, map Printful variants, and save permanent mockup images.</p><Button onClick={() => updateDesign(d.id, { stage: 'parent_approval', variants_ready: true } as any)}>Send sample product preview</Button></>}{d.stage === 'ready' && <><ul className="studio-checklist"><li>○ Verified parent consent</li><li>○ Payout account ready</li><li>○ Share rate set in pricing worksheet</li><li>{d.variants_ready ? '✓' : '○'} Product preview approved</li></ul><Button disabled={!publicationReady({ stage: d.stage, verifiedConsent: true, payoutReady: true, rateSet: share !== null && share >= 0 && share <= 100, variantsReady: d.variants_ready })} onClick={() => updateDesign(d.id, { stage: 'published' } as any)}>Publish sample listing</Button></>}</Panel>)}
    {archivedDesigns.length > 0 && <Panel title="Archived by creators"><p className="studio-muted">These designs were archived by their creators. Published artwork can still be sold under the Rendered Youth brand. Only admin can permanently delete.</p>{archivedDesigns.map(d => <div key={d.id} className="studio-row-wrapper archived"><div className="studio-row"><span><strong>{d.title}</strong><small>{stageLabels[d.stage]}</small></span><span className="studio-badge" style={{ background: '#f0e6d0', color: '#8a6d3b' }}>Archived</span></div><button className="studio-row-action" style={{ color: '#c00' }} title="Delete permanently" onClick={() => setAdminDeleteId(d.id)}><Trash2 size={16} /></button></div>)}</Panel>}
    {adminDeleteId && <div className="studio-overlay" onClick={() => setAdminDeleteId(null)}><div className="studio-confirm" onClick={e => e.stopPropagation()}><h2>Permanently delete this design?</h2><p>This removes the design, all generated artwork, and any linked products forever. This cannot be undone.</p><div className="studio-actions"><Button disabled={adminDeleting} onClick={() => adminPermanentDelete(adminDeleteId)}>{adminDeleting ? 'Deleting...' : 'Delete permanently'}</Button><button className="studio-link" onClick={() => setAdminDeleteId(null)}>Cancel</button></div></div></div>}
  </>;
}
